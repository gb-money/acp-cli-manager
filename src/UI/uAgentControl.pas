...
begin
  ...
  FAgentList := TDictionary<TAgentType, IACPAgent>.Create;
...
begin
  for LHandler in FHandlers.Values do
    LHandler.Free;
  FHandlers.Free;
  FAgentList.Free;
...
begin
  ...
  begin
    ...
    AAgent.SetSessionManager(FSessionMgr);
...
begin
  ...
  if Supports(Sender, IACPAgent, LAgent) then
  begin
    ...
    begin
      ...
      LAgent.SetSessionLogPath(LSid, LSession.LogPath);
...
begin
  // Implementation for UI reflection of file writes could go here
end;

procedure TAgentControl.OnAgentStateChange(Sender: TObject; const OldState, NewState: TAgentState);
begin
  // Implementation for UI state changes (indicator, etc) could go here
end;
...
begin
  LSession := FSessionMgr.GetSessionById(SessionId);
  if Assigned(LSession) and Supports(Sender, IACPAgent, LAgent) then
  begin
    if LSession.IsThoughtStreaming then EndStreaming(SessionId, 'thought');
    if LSession.IsMessageStreaming then EndStreaming(SessionId, 'message');

    LHandler := GetHandler(LSession.AgentType, LAgent);
...
var
  LCaptured: TSessionInfo;
begin
  LCaptured := ASession;
  TThread.Queue(nil, TThreadProcedure(procedure
  begin
    if Assigned(LCaptured) then
    begin
...
begin
  ...
  begin
    if Assigned(FHandlerFactory) then
    begin
      Result := TAgentHandler(FHandlerFactory(AType, AAgent, Self));
...
begin
  ...
  begin
    ...
    if Assigned(FAgentFactory) then
      LAgent := FAgentFactory(LType)
    else
      LAgent := nil;

    if not Assigned(LAgent) then Exit;
    ...
    LAgent.SetWorkspace(LSelectedDir);
...
var
  LSid: string;
  LTarget: TSessionInfo;
  LAgent: IACPAgent;
begin
  if Params.TryGetValue('id', LSid) then
  begin
    LTarget := FSessionMgr.GetSessionById(LSid);
    if Assigned(LTarget) and Assigned(LTarget.Agent) and Supports(LTarget.Agent, IACPAgent, LAgent) then
    begin
       LTarget.IsLoading := True;
       LTarget.IsRestoring := True;
       UpdateSession(LTarget);
       HandleSelectSession(LTarget);
       
       LAgent.SetWorkspace(LTarget.Cwd);
       LAgent.ResumeSession(LSid);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(LActive) and Assigned(LActive.Agent) and Supports(LActive.Agent, IACPAgent, LAgent) then
    begin
      LAgent.SendPrompt(LActive.SessionId, LText);
...
begin
  if Params.TryGetValue('id', LSid) then
    HandleSelectSession(FSessionMgr.GetSessionById(LSid));
end;
...
begin
  ...
  begin
    LActive := FSessionMgr.ActiveSession;
    if Assigned(LActive) and Supports(LActive.Agent, IACPAgent, LAgent) then
      LAgent.ChangeModel(LActive.SessionId, LModelId);
  end;
end;
...
begin
  ...
  begin
    LActive := FSessionMgr.ActiveSession;
    if Assigned(LActive) and Supports(LActive.Agent, IACPAgent, LAgent) then
      LAgent.ReplyPermission(LID, LActive.SessionId, LOptionId);
  end;
end;
...
begin
  ...
  if Assigned(LActive) and Supports(LActive.Agent, IACPAgent, LAgent) then
  begin
    LAgent.CancelPrompt(LActive.SessionId);
    ShowTyping(False);
  end;
end;
...
var
  LRelPath, LFullPath, LCallbackId, LTargetRoot: string;
  LActive: TSessionInfo;
begin
...
begin
  ...
  for LAgent in FAgentList.Values do
  begin
    LSessionIds := LAgent.GetSessionList;
    for LSid in LSessionIds do
    begin
      if Assigned(FAgentFactory) then
        LNewAgent := FAgentFactory(LAgent.AgentType)
      else
        LNewAgent := nil;

      if Assigned(LNewAgent) then
        RegisterAgent(LAgent.AgentType, LNewAgent)
      else
        LNewAgent := LAgent;

      LSession := FSessionMgr.AddSession(LNewAgent, LAgent.AgentType, LSid, LSid);
      if Assigned(LSession) then
      begin
        LSession.LoadMetadata; 
        if Assigned(LNewAgent) then
          LNewAgent.SetSessionLogPath(LSid, LSession.LogPath);
      end;
    end;
  end;
  ...
  try
    ...
    try
      for LSession in LSessions do
        LArray.AddObject.Assign(SessionToJSON(LSession));
    finally
      LSessions.Free;
    end;
    LJson := LArray.ToJSON(False);
...
begin
  ...
  try
    LBase64 := TNetEncoding.Base64.EncodeBytesToString(TEncoding.UTF8.GetBytes(LObj.ToJSON(False))).Replace(#13, '').Replace(#10, '');
    ExecuteJS('window.ACP.updateSession("' + LBase64 + '")');
  finally
    LObj.Free;
  end;
end;
...
begin
  ...
  try
    ...
    if AStopReason <> '' then LObj.S['stopReason'] := AStopReason;
    ExecuteJS('window.ACP.streamMessage(' + LObj.ToJSON(False) + ')');
  finally
    LObj.Free;
  end;
end;
...
begin
  ...
  try
    ...
    LObj.S['content'] := AContent;
    LObj.S['timestamp'] := FormatDateTime('yyyy-mm-dd hh:nn:ss', Now);
    ExecuteJS('window.ACP.streamThought(' + LObj.ToJSON(False) + ')');
  finally
    LObj.Free;
  end;
end;
...
begin
  TThread.Queue(nil, TThreadProcedure(procedure
  begin
    ExecuteJS('window.ACP.breakGrouping()');
  end));
end;
...
begin
  ...
  try
    ...
    LData.S['id'] := AID;
    LData.S['id_method'] := AMethod;
    if AToolCallJson <> '' then LData.O['toolCall'].FromJSON(AToolCallJson);
    if AOptionsJson <> '' then LData.A['options'].FromJSON(AOptionsJson);
...
begin
  TThread.Queue(nil, TThreadProcedure(procedure
  begin
    ExecuteJS(Format('window.ACP.showProcessing(%s)', [BoolToStr(AShow, True).ToLower]));
  end));
end;
...
begin
  ...
  if not Assigned(ASessionInfo) then Exit;
  
  Result.S['id'] := ASessionInfo.SessionId;
  Result.S['name'] := ASessionInfo.Name;
  Result.S['workspace'] := GetWorkspaceDisplayText(ASessionInfo.Cwd);
  Result.B['active'] := FSessionMgr.ActiveSession = ASessionInfo;
  Result.B['pinned'] := ASessionInfo.IsPinned;
  Result.B['isActive'] := ASessionInfo.IsActive;
  Result.B['isWaitForResponse'] := ASessionInfo.IsWaitForResponse;
  ...
  Result.S['icon'] := 'forum';
  Result.S['lastMsg'] := 'Ready to chat...';
  if ASessionInfo.IsLoading then Result.S['lastMsg'] := 'Starting process...';
  
  if Assigned(ASessionInfo.Agent) and Supports(ASessionInfo.Agent, IACPAgent, LAgent) then
  begin
    var LJsonStr := LAgent.GetSessionsJson(ASessionInfo.SessionId);
    if LJsonStr <> '' then
    begin
      // Models
      var LModels := LAgent.GetModels;
      ...
      begin
        ...
        for var i := 0 to High(LModels) do
        begin
          var LItem := LArr.AddObject;
          LItem.S['modelId'] := LModels[i].ModelId;
          LItem.S['name'] := LModels[i].Name;
          if LModels[i].Description <> '' then
            LItem.S['description'] := LModels[i].Description;
        end;
        Result.S['currentModelId'] := LAgent.ModelId;
      end;

      // Modes
      var LModes := LAgent.GetModes;
      if Length(LModes) > 0 then
      begin
        var LModesObj := Result.O['modes'];
        LModesObj.S['currentModeId'] := LAgent.ModeId;
        var LArr := LModesObj.A['availableModes'];
        for var i := 0 to High(LModes) do
        begin
          var LItem := LArr.AddObject;
          LItem.S['id'] := LModes[i].ModeId;
          LItem.S['name'] := LModes[i].Name;
          if LModes[i].Description <> '' then
            LItem.S['description'] := LModes[i].Description;
        end;
        Result.S['currentModeId'] := LAgent.ModeId;
      end;

      try
        Result.A['commands'].FromJSON(LJsonStr);
      except
      end;
    end;
  end;
